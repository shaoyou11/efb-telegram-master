...
大文件适配、微信登录管理面板、Watchdog 控制、会话标题修复及家庭 NAS 环境所需功能。
...
``/status`` 显示 Bridge 队列汇总，并提供以下运维入口：

- 「投递明细」：查看待处理附件和失败投递。
- 「异常中心」：查看失败记录、磁盘和映射异常。
- 「失败诊断」：读取 Watchdog 持久化的最新一张失败画面。
- 「Bridge 队列」：查看活动队列和死信队列。
- 「刷新」和「关闭」：更新或关闭当前状态页。

「投递明细」按页列出待处理附件和失败投递。待处理记录可以查看、确认后立即投递或删除；立即投递会跳过附件稳定等待，
删除只移除 ComWechat 待发记录，不删除微信原文件。失败记录可以查看、确认后重新投递或删除；删除只清理 EFB 的失败记录
和失败副本，不影响微信原文件。
...
- 已绑定普通微信联系人时显示通用微信会话命令；已绑定微信群时增加群成员、提醒和改群名等群管理命令。
- Telegram 论坛群会合并全部话题的可用命令；只要存在微信群话题，整个论坛群菜单就会显示微信群管理命令。
- 绑定、解绑、群组升级迁移和 EFB 重启后自动刷新对应群组菜单，无需逐个手动设置。
...
``/bridge`` 只允许 ``admins`` 中的管理员在私聊中使用。菜单可以查看活动队列和死信队列，并提供分页、刷新和隐藏按钮。
活动消息中的 ``staged`` 表示仍在等待附件或排序，``inflight`` 表示正在被 EFB 处理；这两类状态不会被批量重试或放弃。

管理开关默认关闭，状态持久化在 ``/data/operations/state/bridge-queue-settings.json``。
关闭时仍可以查看队列，但重试、重新投递和放弃操作都会被拦截。开启后，单条和批量操作仍然需要再次点击确认。
活动队列还提供「重试全部待投递」和「放弃全部可处理」操作，处理中消息不会被强行改动。
死信提醒共用同一个管理开关，并需要二次确认。

「放弃」不是直接删除 SQLite 记录：Bridge 会在事务中保留去重标记和最小审计字段，清空消息正文并将记录标记为
``discarded``，避免同一消息被重复写入。过期的放弃记录按死信保留周期清理；部署更新不会自动处理现有死信，需从菜单手动选择。

文件传输边界
~~~~~~~~~~~~

启用本地 Bot API 后，本分支会跳过 ETM 对公网 Bot API 默认文件大小的提前拦截。实际传输能力仍取决于本地 Bot API、
Telegram 服务端、存储空间和网络状况。
