...
``/status`` 综合状态页显示 Bridge 队列汇总，并保留“投递明细”“异常中心”“失败诊断”三个运维入口；
按钮按两排三列排列为“投递明细、异常中心、失败诊断”和“Bridge 队列、刷新、关闭”。失败诊断只读取
Watchdog 持久化的最新一张画面，不会上传其他配置内容。

“投递明细”会按页列出待处理附件和失败投递。待处理记录可以查看、确认后立即投递或删除；
立即投递会跳过附件稳定等待，删除只移除 ComWechat 待发记录，不删除微信原文件。失败记录可以
查看、确认后重新投递或删除；删除只清理 EFB 的失败记录和失败副本，不影响微信原文件。
...
``/bridge`` 只允许 ``admins`` 中的管理员在私聊中使用。菜单可以查看活动队列和死信队列，
并提供分页、刷新和隐藏按钮。活动消息中的 ``staged`` 表示仍在等待附件或排序，
``inflight`` 表示正在被 EFB 处理，这两类状态不会被批量重试或放弃。

菜单中的管理开关默认关闭，状态持久化在
``/data/operations/state/bridge-queue-settings.json``。关闭时仍然可以查看队列，
但所有重试、重新投递和放弃操作都会被拦截。开启开关后，单条操作和批量操作仍然
需要再次点击确认。活动队列还提供“重试全部待投递”和“放弃全部可处理”操作，
处理中消息不会被强行改动。死信提醒也共用这个管理开关，并需要二次确认。
