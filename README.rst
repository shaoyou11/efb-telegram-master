...
本分支的 ``/status`` 用于查看运行时间、部署平台、镜像构建时间、GHCR ``latest``
是否与当前运行镜像一致、配置同步检查、备份校验、队列最近延迟以及近 24 小时投递统计。
统计只保存时间、类型和延迟，不保存消息正文、文件名或聊天内容。

状态页中的“投递明细”“异常中心”“失败诊断”均提供关闭入口；失败附件只在需要重试时保留。
手动更新镜像前应先排空投递队列，再按 ``comwechat -> telegram-bot-api/watchdog -> efb``
顺序维护。Watchdog 的自动恢复由独立容器负责，连续失败达到阈值后暂停当前来源，不会无限重启微信。

本地 Telegram Bot API 只改变文件上传和下载路径，不改变 Telegram 云端聊天记录的多设备同步。
EFB 映射数据库、会话状态和媒体缓存仍必须通过 Docker Compose 持久化挂载。
